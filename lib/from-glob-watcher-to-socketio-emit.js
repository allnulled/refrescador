...
module.exports = function (userConfig = {}) {
  ...
  const printConfigurations = function() {
    ...
    color1(`   - payloadFile: ${colors.endToken}${config.payloadFile ? listSeparator + config.payloadFile : "(none)"}`);
        ...
        try {
          ...
          if (config.execute.length) {
            for (let index = 0; index < config.execute.length; index++) {
              ...
              colorWarn(`┌──────────────────────────────────────#`);
              colorWarn(`│ ⚡️ Executing ${(index + 1)}/${config.execute.length}...`);
              colorWarn(`│      - ${command}`);
              colorWarn(`└──────────────────────────────────────#`);
              colorWarn(`🟨🟨🟨🟨🟨🟨🟨🟨🟨🟨🟨🟨🟨🟨🟨🟨🟨🟨🟨`);
              const init = new Date();
              try {
                ...
                colorSuccess(`🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩`);
                colorSuccess(`┌──────────────────────────────────────#`);
                colorSuccess(`│ 💫 Executed successfully ${(index + 1)}/${config.execute.length}! 🎊`);
                colorSuccess(`│      - ${command}`);
                const diff = (new Date()) - init;
                timings.push({ command, diff });
                colorSuccess(`│ ⏳ ${diff / 1000} seconds for the execution 💫`);
                colorSuccess(`└──────────────────────────────────────#`);
              } catch (error) {
                colorError(`🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥🟥`);
                colorError(`┌──────────────────────────────────────#`);
                ...
                const diff = (new Date()) - init;
                timings.push({ command, diff });
                colorError(`│ ⏳ ${diff / 1000} seconds for the execution with errors ⚠️`);
                colorError(`└──────────────────────────────────────#`);
              }
            }
          }
          ...
          if (timings.length) {
            ...
            for (let index = 0; index < timings.length; index++) {
              ...
              colorInform(`│     ⌛️ ➕ ${timing.diff / 1000} seconds = command[${index}] = ${timing.command.substr(0, 40).replace(/\n/g, " ")}`);
              atomicSummatory += timing.diff;
            }
            colorInform(`│     ⌛️ 🟰 ${atomicSummatory / 1000} seconds in summatory`);
          }
          colorInform(`│ 🌏⏳ ${(diffEvent - config.debounce) / 1000} seconds | ${diffEvent / 1000} seconds with debounce of ${config.debounce} milliseconds for the whole event`);
          colorInform(`└──────────────────────────────────────#`);
