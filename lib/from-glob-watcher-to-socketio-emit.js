...
module.exports = function (userConfig = {}) {
  ...
  const triggerReload = function (filepath, event) {
    ...
    return new Promise((resolve, reject) => {
      ...
      timeout = setTimeout(async () => {
        ...
        try {
          console.clear();
          const operationMessage = event.operation === "add" ? ("🌀ADD=" + rootedPath) :
            event.operation === "edit" ? ("🌀CHANGE=" + rootedPath) :
              event.operation === "delete" ? ("🌀UNLINK=" + rootedPath) : null;
          colorInform(colors.box(operationMessage));
          printUrls();
          // console.log("♻️");
          colorWarn(`♻️  Changes detected on: 📄=${shortenPath(filepath)}`);
          ...
          if (timings.length) {
            ...
            for (let index = 0; index < timings.length; index++) {
              ...
              color1(`  ⌛️ ➕ ${timing.diff / 1000}s [💻=${timing.command.replace(/\n/g, " ")}]`);
              atomicSummatory += timing.diff;
            }
            color1(`  ⌛️ 🟰 ${atomicSummatory / 1000}s in summatory`);
          }
          color1(`  🌏 ⏳ ${(diffEvent - config.debounce) / 1000}s | ${diffEvent / 1000}s with debounce of ${config.debounce} milliseconds for the whole event`);
  ...
  const matchesIgnoreCallback = function (filepath) {
    const rootedPath = filepath.startsWith(config.basedir + "/") ? filepath.replace(config.basedir + "/", "@/") : filepath;
    try {
      // Adaptado para que pueda ser un callback o un json/array
      ...
      if (Array.isArray(ignoredEntity)) {
        ...
        throw new Error(`Ignored pattern must export array or function on «${config.ignoreCallback}» but «${typeof ignoredEntity}» was found instead`);
      }
    } catch (error) {
      console.error(`Error loading ignore callback file «${rootedPath}»:`, error);
    }
  };
  ...
  const printUrls = function () {
    ...
    color2(`🔔 Puntos disponibles: ⏳=${dateString} 📂=${config.basedir}`);
    ...
    if (extraControllers.length) {
      for (let indexCtrlReport = 0; indexCtrlReport < extraControllers.length; indexCtrlReport++) {
        ...
        color2(` 🔹 ${('[' + extraController[0] + ']').padEnd(13)} http://localhost:${config.port}` + normalizeJoin(extraController[1]));
      }
    }
  }
