...
const assertion = require("./from-condition-to-error.js");
const fromKebabCaseToCamelCase = require("./from-kebab-case-to-camel-case.js");

module.exports = function (configurations = {}, args = process.argv.slice(2)) {
  const output = {};
  let current = "_";
  const aliases = {};
  const arePositionalsForbidden = ("_" in configurations) && (configurations._ === false);
  delete configurations._;
  assertion(Array.isArray(args), "Parameter «args» must be array on «from-cli-args-to-map»");
  // -------------------------
  // Extraer aliases
  // -------------------------
